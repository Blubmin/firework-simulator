#include "Program.h"

#include <iostream>
#include <cassert>
#include <vector> 

#include "GLSL.h"

using namespace std;

Program::Program() :
	vShaderName(""),
	fShaderName(""),
	pid(0),
	verbose(true)
{
	
}

Program::~Program()
{
	
}

void Program::setShaderNames(const string &v, const string &f, const string &g)
{
	vShaderName = v;
	fShaderName = f;
    gShaderName = g;
}

bool Program::init()
{
	GLint rc;
	
	// Create shader handles
	GLuint VS = glCreateShader(GL_VERTEX_SHADER);
    GLuint GS = -1;
    if (gShaderName != "")
        GS = glCreateShader(GL_GEOMETRY_SHADER);
	GLuint FS = glCreateShader(GL_FRAGMENT_SHADER);
	
	// Read shader sources
	const char *vshader = GLSL::textFileRead(vShaderName.c_str());
    const char *gshader;
    if (gShaderName != "")
        gshader = GLSL::textFileRead(gShaderName.c_str());
	const char *fshader = GLSL::textFileRead(fShaderName.c_str());
    
	glShaderSource(VS, 1, &vshader, NULL);
    if (gShaderName != "")
        glShaderSource(GS, 1, &gshader, NULL);
	glShaderSource(FS, 1, &fshader, NULL);
    
	// Compile vertex shader
	glCompileShader(VS);
	glGetShaderiv(VS, GL_COMPILE_STATUS, &rc);
	if(!rc) {
		if(isVerbose()) {
			cout << "Error compiling vertex shader " << vShaderName << endl;
            GLSL::printShaderInfoLog(VS);
		}
		return false;
	}
    
    // Compile geometry shader
    if(gShaderName != "") {
        glCompileShader(GS);
        glGetShaderiv(GS, GL_COMPILE_STATUS, & rc);
        if(!rc) {
            if (isVerbose()) {
                cout << "Error compiling geometry shader " << gShaderName << endl;
                GLSL::printProgramInfoLog(GS);
            }
        }
    }
	
	// Compile fragment shader
	glCompileShader(FS);
	glGetShaderiv(FS, GL_COMPILE_STATUS, &rc);
	if(!rc) {
		if(isVerbose()) {
			cout << "Error compiling fragment shader " << fShaderName << endl;
            GLSL::printShaderInfoLog(FS);
		}
		return false;
	}
	
	// Create the program and link
	pid = glCreateProgram();
	glAttachShader(pid, VS);
    if (gShaderName != "")
        glAttachShader(pid, GS);
	glAttachShader(pid, FS);
	glLinkProgram(pid);
	glGetProgramiv(pid, GL_LINK_STATUS, &rc);
	if(!rc) {
		if(isVerbose()) {
			GLSL::printProgramInfoLog(pid);
            if (gShaderName == "")
                cout << "Error linking shaders " << vShaderName << " and " << fShaderName << endl;
            else
                cout << "Error linking shaders " << vShaderName << ", " + gShaderName + ", and " << fShaderName << endl;
		}
		return false;
	}
	
    setupHandles();
    
	GLSL::checkError(GET_FILE_LINE);
	return true;
}

void Program::bind()
{
	glUseProgram(pid);
}

void Program::unbind()
{
	glUseProgram(0);
}

GLint Program::getAttribute(const string &name) const
{
	map<string,GLint>::const_iterator attribute = attributes.find(name.c_str());
	if(attribute == attributes.end()) {
		if(isVerbose()) {
			cout << name << " is not an attribute variable" << endl;
		}
		return -1;
	}
	return attribute->second;
}

GLint Program::getUniform(const string &name) const
{
	map<string,GLint>::const_iterator uniform = uniforms.find(name.c_str());
	if(uniform == uniforms.end()) {
		if(isVerbose()) {
			cout << name << " is not a uniform variable" << endl;
		}
		return -1;
	}
	return uniform->second;
}

void Program::setupHandles()
{
    int total = -1;
    glGetProgramiv(pid, GL_ACTIVE_UNIFORMS, &total);
    for (int i = 0; i<total; ++i)
    {
        int name_len = -1, num = -1;
        GLenum type = GL_ZERO;
        char name[100];
        glGetActiveUniform(pid, GLuint(i), sizeof(name) - 1,
                           &name_len, &num, &type, name);
        name[name_len] = 0;
        GLuint location = glGetUniformLocation(pid, name);
        uniforms.insert(pair<string, GLint>(name, location));
    }
    
    total = -1;
    glGetProgramiv(pid, GL_ACTIVE_ATTRIBUTES, &total);
    for (int i = 0; i<total; ++i)
    {
        int name_len = -1, num = -1;
        GLenum type = GL_ZERO;
        char name[100];
        glGetActiveAttrib(pid, GLuint(i), sizeof(name) - 1,
                          &name_len, &num, &type, name);
        name[name_len] = 0;
        GLuint location = glGetAttribLocation(pid, name);
        attributes.insert(pair<string, GLint>(name, location));
    }
    
    total = -1;
    glGetProgramiv(pid, GL_ACTIVE_UNIFORM_BLOCKS, &total);
    for (int i = 0; i<total; ++i)
    {
        int name_len;
        glGetActiveUniformBlockiv(pid, GLuint(i), GL_UNIFORM_BLOCK_NAME_LENGTH, &name_len);
        std::vector<GLchar> name;
        glGetActiveUniformBlockName(pid, GLuint(i), name_len, NULL, &name[0]);
        
        GLuint location = glGetUniformBlockIndex(pid, &name[0]);
        uniformBlocks.insert(pair<string, GLint>(&name[0], location));
    }
}
